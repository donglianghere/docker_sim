...
#   CONTROL_LAW     默认 trajectory。可选 attitude——切换track_dynus_traj_py的控制律，
#                   见dynus_offboard_node.py里OffboardDynusFollower.__init__的大段注释。
#                   trajectory是唯一实际飞过的模式；attitude代码完整但从没实测飞过，
#                   切换前看README.md。
set -eo pipefail
