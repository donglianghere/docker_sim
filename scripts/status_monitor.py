...
class StatusMonitor(Node):
    ...
    def _redraw(self):
        ...
        for ns in self.namespaces:
            ns_, pos, rpy, thrust, hz = self._row(ns)
            row = (
                f"| {ns_:<6} | {pos:<22} | {rpy:<22} | {thrust:<9} | {hz:<8} |"
            )
            lines.append(row)
        lines.append(sep)
