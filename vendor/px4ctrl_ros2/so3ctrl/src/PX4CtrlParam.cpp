// 从px4ctrl的PX4CtrlParam.cpp搬过来，唯一改动是加了Ki0-2/Kib0-2六行
// read_essential_param调用（对应PX4CtrlParam.h新增的Gain字段），其余
// 一字未改。
...
{
	// 原ROS1版本参数名用"/"分层（如"gain/Kp0"），ROS2版本约定用"."分层，
	...
	// 注意：Kvd0/1/2这三个字段在原px4ctrl版本里也从来没有被读取/赋值过
	// （struct里声明了，yaml里也有Kvd0/1/2，但config_from_ros_handle
	// 从未读取它们），是上游代码本身的遗留状况，这里原样保留、不额外修复。

	// 新增：SO3Control用到的世界系/机体系位置积分增益。
	read_essential_param(node, "gain.Ki0", gain.Ki0);
	read_essential_param(node, "gain.Ki1", gain.Ki1);
	read_essential_param(node, "gain.Ki2", gain.Ki2);
	read_essential_param(node, "gain.Kib0", gain.Kib0);
	read_essential_param(node, "gain.Kib1", gain.Kib1);
	read_essential_param(node, "gain.Kib2", gain.Kib2);
