// 从px4ctrl的input.h原样搬过来，一行没改（Imu_Data_t/Battery_Data_t这两个
// 类型仍然保留——pt4ctrl_node.cpp不会订阅imu/battery话题，这两个类型永远
// 用不到，但保留声明本身无害，换来跟px4ctrl/so3ctrl结构完全对齐、方便
// 以后对照上游改动，不需要额外维护一份精简版）。
#ifndef __INPUT_H
#define __INPUT_H
