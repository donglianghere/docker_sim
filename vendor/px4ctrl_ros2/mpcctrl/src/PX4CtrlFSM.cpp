...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
		{
			...
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_HOVER(L2). No odom!");
				break;
			}
			if (cmd_is_received(now_time))
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_HOVER(L2). You are sending commands before toggling into AUTO_HOVER, which is not allowed. Stop sending commands now!");
				break;
			}
			if (odom_data.v.norm() > 3.0)
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_HOVER(L2). Odom_Vel=%fm/s, which seems that the locolization module goes wrong!", odom_data.v.norm());
				break;
			}
			...
			RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[32m");
		}
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::msg::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			if (!odom_is_received(now_time))
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_TAKEOFF. No odom!");
				break;
			}
			if (cmd_is_received(now_time))
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_TAKEOFF. You are sending commands before toggling into AUTO_TAKEOFF, which is not allowed. Stop sending commands now!");
				break;
			}
			if (odom_data.v.norm() > 0.1)
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static takeoff is not allowed!", odom_data.v.norm());
				break;
			}
			if (!get_landed())
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_TAKEOFF. land detector says that the drone is not landed now!");
				break;
			}
			if (rc_is_received(now_time)) // Check this only if RC is connected.
			{
				...
				{
					RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject AUTO_TAKEOFF. If you have your RC connected, keep its switches at \"auto hover\" and \"command control\" states, and all sticks at the center, then takeoff again.");
					while (rclcpp::ok())
					{
						...
						if (rc_data.is_hover_mode && rc_data.is_command_mode && rc_data.check_centered())
						{
							RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] OK, you can takeoff again.\033[32m");
							break;
						}
					}
					break;
			...
			RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] MANUAL_CTRL(L1) --> AUTO_TAKEOFF\033[32m");
		}
		...
		if (rc_data.toggle_reboot) // Try to reboot. EKF2 based PX4 FCU requires reboot when its state estimator goes wrong.
		{
			...
			{
				RCLCPP_ERROR(node_->get_logger(), "[mpcctrl] Reject reboot! Disarm the drone first!");
				break;
			}
			reboot_FCU();
		}
	...
	case AUTO_HOVER:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			RCLCPP_WARN(node_->get_logger(), "[mpcctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)");
		}
		else if (rc_data.is_command_mode && cmd_is_received(now_time))
		{
			if (state_data.current_state.mode == "OFFBOARD")
			{
				state = CMD_CTRL;
				des = get_cmd_des();
				RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[32m");
			}
		}
		else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::msg::TakeoffLand::LAND)
		{
			...
			RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] AUTO_HOVER(L2) --> AUTO_LAND\033[32m");
		}
		else
		{
			set_hov_with_rc();
				...
				(takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second))
			{
				...
				RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] TRIGGER sent, allow user command.\033[32m");
			}
	...
	case CMD_CTRL:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			RCLCPP_WARN(node_->get_logger(), "[mpcctrl] From CMD_CTRL(L3) to MANUAL_CTRL(L1)!");
		}
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
			RCLCPP_INFO(node_->get_logger(), "[mpcctrl] From CMD_CTRL(L3) to AUTO_HOVER(L2)!");
		}
		else
		{
			des = get_cmd_des();
		...
		if (state == CMD_CTRL && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::msg::TakeoffLand::LAND)
		{
			...
			RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] CMD_CTRL(L3) --> AUTO_LAND\033[32m");
		}
	...
	case AUTO_TAKEOFF:
	{
		...
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			RCLCPP_WARN(node_->get_logger(), "[mpcctrl] AUTO_TAKEOFF --> MANUAL_CTRL(L1)");
		}
		...
		else if (odom_data.p(2) >= (takeoff_land.start_pose(2) + param.takeoff_land.height)) // reach the desired height
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] AUTO_TAKEOFF --> AUTO_HOVER(L2)\033[32m");
	...
	case AUTO_LAND:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			RCLCPP_WARN(node_->get_logger(), "[mpcctrl] From AUTO_LAND to MANUAL_CTRL(L1)!");
		}
		else if (!rc_data.is_command_mode)
		{
			...
			set_hov_with_odom();
			des = get_hover_des();
			RCLCPP_INFO(node_->get_logger(), "[mpcctrl] From AUTO_LAND to AUTO_HOVER(L2)!");
		}
		else if (!get_landed())
		{
			des = get_takeoff_land_des(-param.takeoff_land.speed);
		...
		else
		{
			...
			if (print_once_flag)
			{
				RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] Wait for abount 10s to let the drone arm.\033[32m");
			...
			if (extended_state_data.current_extended_state.landed_state == mavros_msgs::msg::ExtendedState::LANDED_STATE_ON_GROUND) // PX4 allows disarm after this
			{
				...
				if (now_time.seconds() - last_trial_time > 1.0)
				{
					if (toggle_arm_disarm(false)) // disarm
					{
						...
						RCLCPP_INFO(node_->get_logger(), "\033[32m[mpcctrl] AUTO_LAND --> MANUAL_CTRL(L1)\033[32m");
					}
...
bool PX4CtrlFSM::toggle_offboard_mode(bool on_off)
{
	...
	// 而不用担心递归spin死锁：process()本身是从px4ctrl_node.cpp里那个手写的
