...
void RC_Data_t::check_validity()
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("mpcctrl"), "RC data validity check fail. mode=%f, gear=%f, reboot_cmd=%f", mode, gear, reboot_cmd);
    }
}
...
// #define VEL_IN_BODY
    ...
    static int count = 0;
    if (count++ % 500 == 0)
        RCLCPP_WARN(rclcpp::get_logger("mpcctrl"), "VEL_IN_BODY!!!");
#endif
    ...
    if ( (now - last_clear_count_time).seconds() > 1.0 )
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("mpcctrl"), "ODOM frequency seems lower than 100Hz, which is too low!");
        }
        one_min_count = 0;
...
void Imu_Data_t::feed(sensor_msgs::msg::Imu::ConstSharedPtr pMsg)
{
    ...
    if ( (now - last_clear_count_time).seconds() > 1.0 )
    {
        if ( one_min_count < 100 )
        {
            RCLCPP_WARN(rclcpp::get_logger("mpcctrl"), "IMU frequency seems lower than 100Hz, which is too low!");
        }
        one_min_count = 0;
        last_clear_count_time = now;
    }
...
void Battery_Data_t::feed(sensor_msgs::msg::BatteryState::ConstSharedPtr pMsg)
{
    ...
    if (percentage > 0.05)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("mpcctrl"), "[mpcctrl] Voltage=%.3f, percentage=%.3f", volt, percentage);
    ...
    else
    {
        ...
        {
            // RCLCPP_ERROR(rclcpp::get_logger("mpcctrl"), "[mpcctrl] Dangerous! voltage=%.3f, percentage=%.3f", volt, percentage);
            last_print_t = rcv_stamp;
        }
    }
}
