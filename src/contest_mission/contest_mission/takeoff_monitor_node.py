...
class TakeoffMonitorNode(Node):
    def __init__(self):
        ...
        # 能起飞。
        #
        # 2026-09-21 从 1.5 改成 3.0，是实测定的，不是拍脑袋：录 90 秒静止
        # 时的里程计速度序列（69Hz，6174 个样本），算不同窗口下"矢量均值模"
        # 的分布——
        #     窗口    NX01 均值/最大/超0.08占比    NX02
        #     1.5s    0.035 / 0.081 / 0.1%        0.035 / 0.076 / 0%
        #     2.0s    0.034 / 0.078 / 0%          0.034 / 0.073 / 0%
        #     3.0s    0.031 / 0.066 / 0%          0.031 / 0.064 / 0%
        #     8.0s    0.023 / 0.040 / 0%          0.023 / 0.041 / 0%
        # 1.5 秒时 NX01 的最大值 0.081 已经顶破 0.08 门限；3 秒降到 0.066，
        # 留出 20% 余量，代价只有 1.5 秒（相比 PX4 那 70 秒启动可以忽略）。
        #
        # 另外注意那组数里有个现象：8 秒窗口的均值只降到 0.023，没有继续趋
        # 近 0。纯白噪声下 552 个样本应该把 σ 压低 23 倍到 0.002，实测没有，
        # 说明 uwb_imu 融合的速度里含约 0.02m/s 的低频/偏置成分，平均不掉。
        # 这是这套定位源的固有底，门限 0.08 安全地在它之上——但**不要**把
        # 门限往 0.03 这种"看起来更严"的方向调，那是永远达不到的。
