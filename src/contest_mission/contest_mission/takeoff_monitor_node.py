...
失败原因按阶段分开报（result.stage = `armed`/`stable`），这是下沉带来的
另一个收益——原来选手侧只能笼统超时，分不清是"没解锁"还是"解锁了爬升
不到位"，这两种情况的处置完全不同。

**这个节点不做的事**：起飞前置检查（飞控已连接、定位就绪）、声光反馈、
偏航模式设置仍然留在SDK里——那些是决策/提示，不是判定回路，放在选手侧
没有问题，也方便选手自己替换。
...
class TakeoffMonitorNode(Node):
    ...
    def _run(self, goal_handle):
        ...
        self.get_logger().info('已发布TakeoffLand{TAKEOFF}，开始判定起飞完成')
