...
class ZFilterNode(Node):
    ...
    def __init__(self):
        ...
        # ---- 2026-09-03新增的两个省算力开关，见文件头"为什么要这两个开关" ----
        # 处理频率上限[Hz]，0=不限速（保持原有行为）
        self.declare_parameter('max_rate_hz', 0.0)
        # 惰性订阅：输出话题没有订阅者时，连输入都不订阅
        ...
        if self._lazy:
            # 惰性订阅：1Hz轮询自己输出话题的订阅者数，有人看才订阅输入。
            # 收益不止省下本节点的开销——上游`GridMap::publishMapInflate()`
            # 里第一句就是`if (map_inf_pub_->get_subscription_count() <= 0)
            # return;`，所以只要这里不订阅，ego_planner连那3MB点云都不会
            # 构造和序列化，DDS也不用传，是全链路省。
            self.create_timer(1.0, self._lazy_tick)
        else:
            self._cloud_sub = self.create_subscription(
                PointCloud2, input_topic, self._cb, 5)

        self.get_logger().info(
            f'pointcloud_z_filter就绪：{input_topic} -> {output_topic}，'
            f'跟随{odom_topic}的相对高度窗口[{self.min_z}, {self.max_z}]（米）'
            f'，惰性订阅={self._lazy}，频率上限='
            f'{"不限" if self._max_rate_hz <= 0 else str(self._max_rate_hz) + "Hz"}')

    def _lazy_tick(self):
        """有下游订阅者才订阅输入，没人看就退订——退订之后上游也会停止发布。"""
        want = self._pub.get_subscription_count() > 0
        if want and self._cloud_sub is None:
            self._cloud_sub = self.create_subscription(
                PointCloud2, self._input_topic, self._cb, 5)
            self.get_logger().info('检测到下游订阅者，开始订阅输入点云')
        elif not want and self._cloud_sub is not None:
            self.destroy_subscription(self._cloud_sub)
            self._cloud_sub = None
            self._last_proc_t = 0.0
            self.get_logger().info('下游无订阅者，已退订输入点云（上游也会随之停发）')

    def _odom_cb(self, msg: Odometry):
        self._drone_z = msg.pose.pose.position.z

    def _cb(self, msg: PointCloud2):
        # 限速：距上次处理不足一个周期就丢帧。用单调时钟，不受系统时间跳变
        # 影响；丢帧发生在解析之前，省掉的是这一帧的全部处理开销。
